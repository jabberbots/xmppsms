...
def iqIncoming(con,iq): #iq запросы которые получаем
    ...
    if iqInbox.getCDATA() == 'SMS отправлено':
...
def checkSending(): #проверяет дошло ли смс, по ответному iq, которое уповестить что не дошло
    pass
...
while numacc <= 1: #цикл логина двух аккаунтов
    ...
    bot.RegisterHandler('message', messageIncoming) #фукция следит только за первым привязанным объектом
